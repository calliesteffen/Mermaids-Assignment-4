...
library(vcdExtra)
library(kableExtra)
>>>>>>> 01664a10a975b014b97ddb5d96ac4e2b836da946
...
lobster_abund<- select(size_abund,YEAR,SITE,COUNT) %>% 
  count(YEAR,SITE)  ##Making total counts of lobsters at each site for each year

traps_site <-select(traps,YEAR,SITE,TRAPS) %>%    ##(keeping year, site, traps)
  filter(SITE=="AQUE"|SITE=="NAPL"|SITE=="MOHK"|SITE=="IVEE"|SITE=="CARP") %>% 
  count(YEAR,SITE) ##Making total counts of traps at each site for each year
...
###QQ Plot for Traps
traps_qq<-ggplot(traps_site, aes(sample=n)) +
  geom_qq()
traps_qq
...
### Making Line Graps to Compare and Explore if Traps and Lobsters are Associated

line_traps<-ggplot(traps_site,aes(x=YEAR,y=n))+
  geom_point()+
  geom_line(aes(col=SITE)) +
  theme_classic() +
  labs(y="TRAP COUNT") +
  scale_y_continuous(expand=c(0,0), limits = c(0,280), breaks = c(0,20,40,60,80,100,120,140,160,180,200,220,240,260,280))


line_traps

line_lobster<-ggplot(lobster_abund, aes(x=YEAR,y=n))+
  geom_point()+
  geom_line(aes(col=SITE))+
  theme_classic()+
  labs(y="LOBSTER COUNT")+
  scale_y_continuous(expand = c(0,0), limits=c(0,360), breaks=c(0,20,40,60,80,100,120,140,160,180,200,220,240,260,280,300,320,340,360))

line_lobster


###Making a Table for totals of abundance and traps at each site

table_lobster <-kable(combined, col.names = c("YEAR","SITE","Lobster","Traps")) %>%  
...
#create data summary frame of lobster size mean, SD, and sample size 
